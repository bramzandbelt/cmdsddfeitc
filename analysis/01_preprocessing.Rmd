...
preproc_excl_data_dir <- 
  file.path(preproc_data_dir, "excluded")
...
(calibration_indifference_points <- 
  ...
  dplyr::mutate(relative_diff = c(NA, diff(ip))/ 43.52,
                ...
                last_ip_within_bounds = c(rep(TRUE,6),
                                          dplyr::last(ip) > 8 * 0.17 &
                                          dplyr::last(ip) < 43.52 - 8 * 0.17),
...
The directory in which the cleaned data will be written depends on whether all performance criteria were met (`preproc_incl_data_dir`) or not (`preproc_excl_data_dir`).
