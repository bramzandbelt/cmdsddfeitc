...
params:
  ...
  participant_id: 21   # identifier of participant in the study
  ...
  algorithm: "DEoptimR"
  par_vals: !r c(0.80, 1, 0.34, 1, 1.01, 0.93, 0.22, 3.48, 1.30) # when optimize = FALSE, report is generated using these values
...
if (params$visualize) {
 ...
 plt_free_par_vals_per_iter_tibb <- 
   ...
   # Title
    ggplot2::labs(caption = sprintf("task: %s\npid: %.3d\nmodel: %s\npmz: %s",
                                       task_version,
                                       params$participant_id, # pid
                                       params$model_name, # model
 ...
 print(plt_free_par_vals_per_iter_tibb)
}
```
...
if (params$visualize) {
  ...
  print(plt_optim_val_per_iter_tibb)
}
...
if (params$visualize) {
  plt_obs_prd_ll_choices_compressed <- 
  ...
  print(plt_obs_prd_ll_choices_compressed)
}
```
...
if (params$visualize) {
  plt_obs_prd_ll_choices_separate <- 
    ggplot2::ggplot(data = prd_pll_for_plot,
                    ggplot2::aes(x = m_s,
                                 y = p_ll, 
                                 color = frame)) +
  ...
  print(plt_obs_prd_ll_choices_separate)
}
```
...
# Filename
fn_plt_free_par_vals_per_iter_tibb <- 
  sprintf("plot_evolution_of_params_across_iters_task-%s_pid-%.3d_model-%s_pmz-%s_bounds-%s_algorithm-%s_BIC-%.0f",
          task_version,
          params$participant_id, # pid
          params$model_name, # mode
          params$parameterization, # pmz
...
ggplot2::ggsave(path = figures_dir,
                filename = paste0(fn_plt_free_par_vals_per_iter_tibb, ".png"),
                plot = plt_free_par_vals_per_iter_tibb,
                width = 18.0,
                units = "cm",
                dpi = dpi)
...
# As vector image --------------------------------------------------------------
ggplot2::ggsave(path = figures_dir,
                filename = paste0(fn_plt_free_par_vals_per_iter_tibb, ".pdf"),
                plot = plt_free_par_vals_per_iter_tibb,
                width = 18.0,
                units = "cm",
                dpi = dpi)

# Optimization function value as a function of iteration =======================

# Filename
fn_plt_optim_fun_val_per_iter <-
    sprintf("plot_evolution_of_fun_val_across_iters_task-%s_pid-%.3d_model-%s_pmz-%s_bounds-%s_algorithm-%s_BIC-%.0f",
            task_version,
            params$participant_id, # pid
            params$model_name, # mode
            params$parameterization, # pmz
...
# As raster image --------------------------------------------------------------
ggplot2::ggsave(path = figures_dir,
                filename = paste0(fn_plt_optim_fun_val_per_iter, ".png"),
                plot = plt_optim_val_per_iter_tibb,
                width = 8.5,
                units = "cm",
                dpi = dpi)
...
# As vector image --------------------------------------------------------------
ggplot2::ggsave(path = figures_dir,
                filename = paste0(fn_plt_optim_fun_val_per_iter, ".pdf"),
                plot = plt_optim_val_per_iter_tibb,
                width = 8.5,
                units = "cm",
                dpi = dpi)
...
# Predicted and observed probability of LL choice - compressed format ==========

# Filename
fn_plt_obs_prd_ll_choices_compressed <-
    sprintf("plot_prd_obs_prob_ll_choice_compressed_task-%s_pid-%.3d_model-%s_pmz-%s_bounds-%s_algorithm-%s_BIC-%.0f",
            task_version,
            params$participant_id, # pid
            params$model_name, # mode
            params$parameterization, # pmz
...
# As raster image --------------------------------------------------------------
ggplot2::ggsave(path = figures_dir,
                filename = paste0(fn_plt_obs_prd_ll_choices_compressed, ".png"),
                plot = plt_obs_prd_ll_choices_compressed,
                width = 18.0,
                units = "cm",
                dpi = dpi)
...
# As vector image --------------------------------------------------------------
ggplot2::ggsave(path = figures_dir,
                filename = paste0(fn_plt_obs_prd_ll_choices_compressed, ".pdf"),
                plot = plt_obs_prd_ll_choices_compressed,
                width = 18.0,
                units = "cm",
                dpi = dpi)

# Predicted and observed probability of LL choice - expanded format ============

# Filename
fn_plt_obs_prd_ll_choices_separate <-
    sprintf("plot_prd_obs_prob_ll_choice_separate_task-%s_pid-%.3d_model-%s_pmz-%s_bounds-%s_algorithm-%s_BIC-%.0f",
            task_version,
            params$participant_id, # pid
            params$model_name, # mode
            params$parameterization, # pmz
...
# As raster image --------------------------------------------------------------
ggplot2::ggsave(path = figures_dir,
                filename = paste0(fn_plt_obs_prd_ll_choices_separate, ".png"),
                plot = plt_obs_prd_ll_choices_separate,
                width = 18.0,
                units = "cm",
                dpi = dpi)
...
# As vector image --------------------------------------------------------------
ggplot2::ggsave(path = figures_dir,
                filename = paste0(fn_plt_obs_prd_ll_choices_separate, ".pdf"),
                plot = plt_obs_prd_ll_choices_separate,
                width = 18.0,
                units = "cm",
                dpi = dpi)

# Predicted and observed median response time ==================================

# Filename
fn_plt_obs_prd_rt <-
    sprintf("plot_prd_obs_rt_task-%s_pid-%.3d_model-%s_pmz-%s_bounds-%s_algorithm-%s_BIC-%.0f",
            task_version,
            params$participant_id, # pid
            params$model_name, # mode
            params$parameterization, # pmz
...
# As raster image --------------------------------------------------------------
ggplot2::ggsave(path = figures_dir,
                filename = paste0(fn_plt_obs_prd_rt, ".png"),
                plot = plt_obs_prd_rt,
                width = 18.0,
                units = "cm",
                dpi = dpi)
...
# As vector image --------------------------------------------------------------
ggplot2::ggsave(path = figures_dir,
                filename = paste0(fn_plt_obs_prd_rt, ".pdf"),
                plot = plt_obs_prd_rt,
                width = 18.0,
                units = "cm",
                dpi = dpi)
...
fn_plt_prd_ip <-
    sprintf("plot_predicted_indifference_points_task-%s_pid-%.3d_model-%s_pmz-%s_bounds-%s_algorithm-%s_BIC-%.0f",
            task_version,
            params$participant_id, # pid
            params$model_name, # mode
            params$parameterization, # pmz
...
# As raster image --------------------------------------------------------------
ggplot2::ggsave(path = figures_dir,
                filename = paste0(fn_plt_prd_ip, ".png"),
                plot = plt_prd_ip,
                width = 8.5,
                units = "cm",
                dpi = dpi)
...
# As vector image --------------------------------------------------------------
ggplot2::ggsave(path = figures_dir,
                filename = paste0(fn_plt_prd_ip, ".pdf"),
                plot = plt_prd_ip,
                width = 8.5,
                units = "cm",
                dpi = dpi)
