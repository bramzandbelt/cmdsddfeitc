...
params:
  ...
  algorithm: "DEoptimR"
  par_vals: !r c(1.93, 1, 0.128, 1, 1.01, 0.843, 0.00210, 2.48, 1.30) # when optimize = FALSE, report is generated using these values
  max_iter: 2500 # maximum number of iterations
  rel_tol: 0.000001 # relative tolerance
  n_pop_per_free_param: 10
  optimize: TRUE
  visualize: FALSE
  pars_from_file: FALSE
...
if (params$optimize) {
  ...
  if (params$algorithm == "DEoptim") {
    ...
    # This quick and dirty way of iterations ensures that we can retrieve information about the evolution of parameter values and function value during optimization, which is not otherwise provided by DEoptimR::JDEoptim
  ...
  if (params$pars_from_file) {
    par_vals_file <-
      list.files(path = converged_models_dir,
                 pattern = sprintf("^best_fitting_params_task-.*_pid-%.3d_model-%s_pmz-%s_bounds-%s.*.csv$",
    ...
    best_pars <-
      readr::read_csv(file = file.path(converged_models_dir, par_vals_file),
    ...
    evol_fun_val_file <- 
      list.files(path = converged_models_dir,
                 pattern = sprintf("^evolution_of_fun_val_across_iters_task-.*_pid-%.3d_model-.*_pmz-%s_bounds-%s.*.csv$",
                                   params$participant_id, # pid
                                   # params$model_name, # model
                                   params$parameterization, # pmz
                                   params$bound_setting # bounds
    ...
    evol_par_val_file <- 
      list.files(path = converged_models_dir,
                 pattern = sprintf("^evolution_of_params_across_iters_task-.*_pid-%.3d_model-%s_pmz-%s_bounds-%s_BIC.*.csv$",
                                   params$participant_id, # pid
                                   params$model_name, # model
                                   params$parameterization, # pmz
                                   params$bound_setting # bounds
    ...
    best_pars <- params$par_vals
  }
    
  # Compute log-likelihood
  val <- 
      itchmodel::get_log_likelihood(x = best_pars,
                                    data = obs_nested,
                                    model = params$model_name,
                                    parameterization = parameterization)
  
  # Fake optimization statistics for the rest of the notebook code to work
  if (params$algorithm == "DEoptim") {
...
# Save log-likelihood and best fitting parameter values to variables
if (params$algorithm == "DEoptim") {
...
## Select indifference point data and area-under-the-curve data for plotting and writing to disk

```{r Select indifference point data for plotting and writing to disk}
prd_ip_data <- 
  prds %>%
...
if (params$visualize & params$model_name %in% c("DDM")) {
  ...
  stim_params_d_tmp <- 
    ...
    dplyr::mutate(d = purrr::pmap(.l = list(frame = .$frame,
  ...
  ggplot2::ggplot(rt_data_plot,
    ...
    ggplot2::labs(shape = "Type", color = "Type") +
...
if (params$optimize) {
  ...
  par_vals_per_iter <- 
    tibble::as.tibble(pars_per_iter) %>% 
    dplyr::mutate(participant_id = params$participant_id,
                  model_name = params$model_name,
                  parameterization = params$parameterization,
                  bound_settings = params$bound_setting,
                  iter = 1:n()) %>%
    dplyr::select(participant_id, model_name, parameterization, bound_settings, iter,
                  dplyr::everything())
    
  par_vals_per_iter_file <-
    file.path(data_output_dir,
              sprintf("evolution_of_params_across_iters_task-%s_pid-%.3d_model-%s_pmz-%s_bounds-%s_BIC-%.0f.csv",
                      task_version,
                      params$participant_id, # pid
                      params$model_name, # mode
                      params$parameterization, # pmz
...
# Write evolution of optimization function value across iterations to disk -----
  ...
  optim_fun_val_per_iter <- 
    tibble::tibble(fun_val = optim_val) %>%
    dplyr::mutate(participant_id = params$participant_id,
                  model_name = params$model_name,
                  parameterization = params$parameterization,
                  bound_settings = params$bound_setting,
  ...
  optim_fun_val_per_iter_file <-
    file.path(data_output_dir,
              sprintf("evolution_of_fun_val_across_iters_task-%s_pid-%.3d_model-%s_pmz-%s_bounds-%s_BIC-%.0f.csv",
                      task_version,
                      params$participant_id, # pid
                      params$model_name, # mode
                      params$parameterization, # pmz
