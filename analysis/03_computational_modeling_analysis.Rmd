...
if (params$visualize) {
  ggplot2::ggplot(optim_val_per_iter_tibb,
                  ggplot2::aes(x = iter,
                               y = fun_val)) +
    ggplot2::geom_step() + 
    ggplot2::scale_x_continuous(name = "Iteration") +
...
if (params$visualize & params$model_name %in% c("DDM", "DFT_CRT")) {
  ...
  my_obs <- rt_data_plot %>% dplyr::filter(type == "obs") %>% dplyr::ungroup()
  my_prds <- rt_data_plot %>% dplyr::filter(type == "prd") %>% dplyr::ungroup()
  
  ggplot2::ggplot(my_obs,
                  ggplot2::aes(x = t_l,
                               y = med_rt,
                               color = m_s_cat)
                  ) +
    ...
    ggplot2::geom_line(data = my_prds,
