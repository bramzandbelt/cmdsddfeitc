...
if (params$optimize) {
  ...
  # Do not optimize, but use parameter values specified in params$par_vals
...
if (params$visualize & params$optimize) {
 h_line_data <- 
    free_par_bounds_tibb %>%
...
if (params$optimize) {
  ...
  par_vals_per_iter <- 
    ...
    dplyr::mutate(participant_id = params$participant_id,
                  model_name = params$participant_id,
                  parameterization = parameterization,
                  bound_settings = params$bound_setting,
                  iter = 1:n()) %>%
