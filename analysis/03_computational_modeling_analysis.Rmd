...
# Make sure that algorithm has correct upper and lower cases
if (stringr::str_to_lower(params$algorithm) == "deoptimr") {
  params$algorithm = "DEoptimR"
} else if (stringr::str_to_lower(params$algorithm) == "deoptim") {
  params$algorithm = "DEoptim"
} else {
  stop(sprintf("Invalid value for params$algorithm: '%s'. Must be either 'DEoptimR' or 'DEoptim'.", params$algorithm))
}
...
if (params$optimize) {
  ...
  if (params$pars_from_file) {
    ...
    optim_out_stats_file <- 
      list.files(path = converged_models_dir,
                 pattern = sprintf("^optim_stats_task-.*_pid-%.3d_model-%s_pmz-%s_bounds-%s_algorithm-%s_BIC.*.csv$",
    ...
    assertthat::assert_that(length(optim_out_stats_file) == 1,
                            msg = "Multiple optimization output files (optim_out_stats_file) detected; unclear which one to select.")
    
    
    optim_out_stats <- 
      readr::read_csv(file.path(converged_models_dir, optim_out_stats_file),
    ...
    optim_val_per_iter_tibb <- 
      readr::read_csv(file.path(converged_models_dir, optim_val_per_iter_file),
                      col_types = readr::cols()) %>%
      tidyr::drop_na() %>%
      dplyr::select(iter, fun_val)
    ...
    best_optim_val <- 
      optim_val_per_iter_tibb %>% 
      dplyr::slice(n()) # Last entry is best value
    ...
    par_vals_per_iter <- 
      readr::read_csv(file.path(converged_models_dir, par_vals_per_iter_file),
                      col_types = readr::cols()) %>%
      dplyr::select(iter, as.character(par_names)) %>%
      tidyr::drop_na()
  ...
  if (params$algorithm == "DEoptim") {
    optim_out <- 
    list(member = list(bestvalit = best_optim_val$fun_val,
         ...
         optim = list(bestval = best_optim_val$fun_val,
    ...
    optim_out <- 
      list(value = best_optim_val$fun_val,
           par = best_par_val,
           iter = ifelse(params$pars_from_file,
                         nrow(par_vals_per_iter),
                         1),
           convergence = ifelse(params$pars_from_file,
                                optim_out_stats$converged,
                                FALSE)
           )
  }
}
